...
function agregar_img_por_zona()
{
  ...
  //DEBUG
  debug_array('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');
  debug_array('CARGAR IMAGENES');
  ...
  $attachments = array();
    ...
    //DEBUG
    debug_array('Array armado de imagenes, el key es la ZONA ID y el value es el ID de la imagen');
  ...
  if ($query_tec->have_posts()) {
    while ($query_tec->have_posts()) {
      ...
      if (array_key_exists($zona_id_tec, $attachments)) {
        //DEBUG
        debug_array('se encontro la imagen del tecnico: '.$zona_id_tec);
        debug_array('------------');
        ...
        debug_array('No hay imagen del tecnico: '.$zona_id_tec);
        debug_array('------------');
      }
    }
    wp_reset_postdata();
    ...
    debug_array('ERROR:No hay tecnicos');
    debug_array('------------');
  }
